...
import {
  ...
  Menu, Check, AlertTriangle, Clock, Zap, History, Loader2, Wifi,
} from "lucide-react";
...
export default function App() {
  ...
  if (session === undefined) {
    return (
      <div className="min-h-screen bg-[#16181B] flex items-center justify-center text-[#8B8F94]">
...
function BypassShop({ session }) {
  ...
  const openLedger = (code) => { setLedgerCode(code); setTab("ledger"); setNavOpen(false); };
  ...
  return (
    <div className="min-h-screen bg-[#16181B] text-[#ECE8E1] lg:flex">
      {/* ---------- Sidebar ---------- */}
      {navOpen && <div className="fixed inset-0 bg-black/50 z-30 lg:hidden" onClick={() => setNavOpen(false)} />}
      <aside
        className={`fixed lg:static z-40 top-0 left-0 h-full w-64 bg-[#1B1E22] border-r border-[#33373C] flex flex-col transition-transform ${
          navOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
        }`}
      >
        <div className="p-4 border-b border-[#33373C]">
          <div className="text-[#8B8F94] text-[10px] font-bold tracking-[0.25em] uppercase">Jaspare Auto</div>
          <div className="flex items-center gap-2 mt-0.5">
            <Boxes size={20} className="text-[#FFC72C]" />
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium mb-0.5 transition-colors ${
                  active ? "bg-[#FFC72C] text-[#16181B]" : "text-[#8B8F94] hover:bg-[#26292E] hover:text-[#ECE8E1]"
                }`}
              >
                <Icon size={17} />
                <span className="flex-1 text-left">{n.label}</span>
                {n.id === "notify" && lowStockCount > 0 && (
                  <span className={`text-[10px] font-bold rounded-full px-1.5 ${active ? "bg-[#16181B] text-[#FFC72C]" : "bg-[#E8483A] text-white"}`}>
                    {lowStockCount}
                  </span>
                )}
              </button>
        ...
        <div className="p-3 border-t border-[#33373C]">
          <div className="flex items-center gap-1.5 text-[10px] text-[#4FA87A] px-3 mb-1">
            <Wifi size={11} /> Live sync on
          </div>
          <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-md text-sm font-medium text-[#8B8F94] hover:bg-[#26292E] hover:text-[#E8483A]">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="border-b border-[#33373C] bg-[#1B1E22] px-4 py-3 flex items-center gap-3 sticky top-0 z-20">
          <button onClick={() => setNavOpen(true)} className="lg:hidden text-[#8B8F94]">
            <Menu size={22} />
          </button>
          <div className="min-w-0">
            <div className="text-[#8B8F94] text-[10px] font-bold tracking-[0.2em] uppercase">
          ...
          <div className="ml-auto flex items-center gap-3 sm:gap-4 shrink-0">
            <div className="hidden sm:flex items-center gap-1.5 text-[#8B8F94] text-xs">
              <Clock size={13} />
              <span>{now.toLocaleDateString("en-KE", { day: "2-digit", month: "short", year: "numeric" })}</span>
              <span className="text-[#ECE8E1] font-semibold tabular-nums">
                {now.toLocaleTimeString("en-KE", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}
              </span>
            </div>
            <div className="text-right">
              <div className="flex items-center gap-1.5 text-[#ECE8E1] text-sm font-semibold">
                <User size={14} className="text-[#FFC72C]" /> {user}
              </div>
              <div className="text-[10px] text-[#8B8F94]">Logged in as</div>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 p-4 max-w-3xl w-full mx-auto">
          {error && (
            <div className="bg-[#2A1E1B] border border-[#E8483A] text-[#E8483A] rounded-md p-3 text-sm mb-4 flex items-center gap-2">
              <AlertTriangle size={15} /> {error}
            </div>
          )}
          {itemsLoading && (
            <div className="flex items-center gap-2 text-[#8B8F94] text-sm mb-4">
      ...
      {toast && (
        <div
          className={`fixed bottom-5 left-1/2 -translate-x-1/2 px-4 py-2.5 rounded-md shadow-lg text-sm font-medium flex items-center gap-2 border z-50 bp-pop ${
            toast.tone === "warn"
              ? "bg-[#2A1E1B] border-[#E8483A] text-[#E8483A]"
              : "bg-[#1B241E] border-[#4FA87A] text-[#4FA87A]"
          }`}
        >
